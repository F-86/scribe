...
**当文档要封装项目现有能力时**（典型：写 SKILL.md / API 文档 / CLAUDE.md，需要写清"调用哪个脚本/函数/接口"）：用户常常不会主动指出相关代码在哪。此时**主动询问或定位**实现文件——"实现统计的代码在哪几个文件？"或请用户提供路径。**不要在没看过实现的情况下凭空写出"调用相应脚本"这类无法执行的占位流程**（违反「可执行 > 抽象」与「不虚构」）。
...
>
> 例外：**文档要封装项目现有能力时**（如写 SKILL.md/API 文档描述已有功能），需要**有针对性地**定位相关实现文件——这是写出可执行内容的前提，不属于"盲目扫描"。先问用户路径或按功能关键词定向查找，而非全库遍历。
