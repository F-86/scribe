...
- **禁止**把 README 的内容（项目介绍、安装步骤、架构图）复制到指令文件中——指令文件只写给 AI agent，写给人的内容在 README
- **禁止**在指令文件中写具体的代码实现细节（如算法逻辑、数据库表结构）——这些在代码注释或设计文档中
- **禁止**使用模糊的编码约定如"代码要整洁"——必须是可判断的规则（如"异步使用 async/await，禁止回调"）
- **禁止**包含过期的命令或路径——生成时确保所有路径、命令、版本号是项目当前状态
