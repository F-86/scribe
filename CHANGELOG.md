...
### Changed
- 端到端实测发现并修复"覆盖现有文档"盲点:用户说"写个 README"但项目已有 README.md 时,原新建流程无任何存在性检查,会直接生成并可能覆盖用户现有文档(不可逆)。现在第 1 步新建分支前增加「新建前的存在性检查(必做)」——目标文件已存在则先问用户"改/补 还是 重写覆盖",由用户决定;并加入同名核心规则
- 封装型文档流程改为「先给选项」而非默认扫描:写 SKILL.md/API 文档等封装现有能力的文档时,先问用户"项目里是否已有实现代码"——用户确认"有"才**定向**定位文件并据此写可执行内容,"没有"则不扫描、直接编写实现脚本+配套文档,"不确定"按没有处理。避免无谓扫描,决定权交还用户(呼应"先确认、不自作主张")
- 端到端场景实测发现并修复"封装型文档"盲点:当用户要写的文档需封装项目现有能力(如"写个统计 MySQL/Redis 的 skill"),而相关实现代码用户未明说时,原流程倾向"让用户给文档/路径"或"别扫描",易产出"调用相应脚本"这类不可执行的空壳 L2。现在第 3 步明确处理这一分叉;「项目目录需要扫描」补充对应例外
