...
### Changed
- 端到端场景实测发现并修复"封装型文档"盲点:当用户要写的文档需封装项目现有能力(如"写个统计 MySQL/Redis 的 skill"),而相关实现代码用户未明说时,原流程倾向"让用户给文档/路径"或"别扫描",易产出"调用相应脚本"这类不可执行的空壳 L2。现在第 3 步「已有素材」明确要求定位实现源代码,并新增"封装现有能力时主动询问/定向定位实现文件、不凭空写占位流程"的指引;「项目目录需要扫描」补充对应例外
